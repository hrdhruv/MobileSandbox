...
# Beta-Binomial prior hyperparameters (uninformative symmetric prior)
BETA_ALPHA_0 = 2.0   # pseudo-count of "malware" observations
BETA_BETA_0  = 2.0   # pseudo-count of "safe" observations
...
def get_bayesian_risk(perm_key: str) -> float:
    ...
    Prior:     Beta(α₀=2, β₀=2)  — uninformative, symmetric
    ...
    This is exactly the Bayesian update expressed as P(malware|evidence):
        P(A|B) = P(B|A)·P(A) / P(B)
    which under a Beta-Binomial model reduces to the closed-form above.
    """
...
def train_ml_model(csv_path, extra_rows=None):
    ...
    extra_rows: list of (feature_dict, label) tuples derived from feedback
                (used by retrain_from_feedback for data augmentation).
...
def retrain_from_feedback():
    ...
    Malware feedback → class 5 pseudo-sample
    Safe feedback    → class 1 pseudo-sample
    """
...
def analyze_permissions(android_permissions):
    """
    Hybrid 3-signal scoring:
      Signal A — per-permission static weights  (60% weight)
      Signal B — ML model class probabilities   (25% weight)
      Signal C — Beta-Binomial Bayesian signal  (15% weight)
    """
...
def adaptive_update(package_name, android_permissions, is_malware,
    ...
    For each permission p:
      ...
      Posterior mean = α/(α+β) → new risk weight on [0,1] → scaled to [0,10]

    Persists feedback to DB (which is then used automatically in future
    get_bayesian_risk() calls, so no need to explicitly store weight changes).
    """
    ...
    # Also do a lightweight update to the in-memory JSON feature_risk
    stats = db_manager.get_feedback_stats()
    for perm in android_permissions:
        key = _perm_short_key(perm)
        ...
        for feature in list(feature_risk.keys()):
            ...
            if any(p in feature for p in parts if len(p) >= 3):
                # Map 0-10 → 0-100 for the JSON store
